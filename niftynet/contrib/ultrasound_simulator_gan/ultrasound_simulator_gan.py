...
class ImageGenerator(BaseGenerator):
    def __init__(self, load_from_checkpoint=True, name='generator'):
        ...
        self.initializers = {'w': tf.compat.v1.keras.initializers.VarianceScaling(scale=2.0), 'b': tf.compat.v1.constant_initializer(0)}
    ...
    def layer_op(self, random_source, image_size, conditioning, is_training):
        ...
        if spatial_rank == 3:
            def resize_func(x, sz):
                ...
                r1 = tf.image.resize(tf.reshape(x, sz_x[:3] + [-1]), sz[0:2])
                r2 = tf.image.resize(tf.reshape(r1, [sz_x[0], sz[0] * sz[1], sz_x[3], -1]),
                                            [sz[0] * sz[1], sz[2]])
        ...
        def concat_cond(x, i):
            if add_noise:
                noise = [tf.random.normal(x.shape.as_list()[0:-1] + [add_noise], 0, .1)]
            else:
                noise = []
        ...
        with tf.compat.v1.name_scope('noise_to_image'):
            ...
            w1p = tf.compat.v1.get_variable("G_W1p", shape=[noise_size, g_no_0], initializer=w_init)
            b1p = tf.compat.v1.get_variable('G_b1p', shape=[g_no_0], initializer=b_init)
            g_h1p = tf.nn.dropout(tf.nn.relu(tf.matmul(random_source, w1p) + b1p), 1 - (keep_prob_ph))
        ...
        with tf.compat.v1.name_scope('final_image'):
            if add_noise:
                noise = tf.random.normal(g_h5.shape.as_list()[0:-1] + [add_noise], 0, .1)
            ...
            x_sample = tf.nn.dropout(tf.nn.tanh(x_sample), 1 - (keep_prob_ph))
        if self.load_from_checkpoint:
            ...
            [tf.compat.v1.add_to_collection('NiftyNetObjectsToRestore', (r[1], checkpoint_name, r[0]))
             for r in restores]
        return x_sample
...
class ImageDiscriminator(BaseDiscriminator):
    def __init__(self, load_from_checkpoint=True, name='discriminator'):
        ...
        self.initializers = {'w': tf.compat.v1.keras.initializers.VarianceScaling(scale=2.0),
                             'b': tf.compat.v1.constant_initializer(0)}

    def layer_op(self, image, conditioning, is_training):
        conditioning = tf.image.resize(conditioning, [160, 120])
        ...
        with tf.compat.v1.name_scope('fc'):
            ...
            D_Wo = tf.compat.v1.get_variable("D_Wo", shape=[d_nf_o, 1], initializer=w_init)
            D_bo = tf.compat.v1.get_variable('D_bo', shape=[1], initializer=b_init)
