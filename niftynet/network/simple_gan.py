...
class ImageGenerator(TrainableLayer):
  ...
  def layer_op(self, random_source, image_size,is_training):
    ...
    dropout_func = lambda x: tf.nn.dropout(x,1 - (.5))
    ...
    if spatial_rank==2:
      ...
      if upscale=='conv_transpose':
        ...
        conv_t_func=lambda x, Wt, sz, st, p: conv_func(tf.image.resize(x,sz[1:3]),tf.transpose(a=Wt,perm=[0,1,3,2]),[1]*4,p)
      ...
      if upscale=='conv_transpose':
        ...
        def resize3(x,sz):
          r1=tf.image.resize(tf.reshape(x,x.shape.as_list()[:3]+[-1]),sz[1:3])
          r2=tf.image.resize(tf.reshape(r1,[sz[0],sz[1]*sz[2],x.shape.as_list()[3],-1]),[sz[1]*sz[2],sz[3]])
          return tf.reshape(r2,sz[:4]+[x.shape.as_list()[-1]])
        conv_t_func=lambda x, Wt, sz, st, p: conv_func(resize3(x,sz),tf.transpose(a=Wt,perm=[0,1,2,4,3]),[1]*5,p)
    ...
    channel_scale = tf.compat.v1.get_variable('G_scale',shape=[1]*(spatial_rank+1)+[intermediate_sizes[-1][-1]],initializer=tf.compat.v1.constant_initializer(.1))
    ...
    channel_shift = tf.compat.v1.get_variable('G_shift',shape=[1]+intermediate_sizes[-1],initializer=tf.compat.v1.constant_initializer(0.))
  ...
  def layer_op(self, image,is_training):
    ...
    if spatial_rank==2:
      ...
      if downscale=='conv_stride':
        ...
        conv_s_unit = lambda x,W: acti_func(norm_func(conv_func(tf.image.resize(x,[x.shape.as_list()[1]//2,x.shape.as_list()[2]//2]), W, [1]+[1]*spatial_rank+[1], "SAME")))
    elif spatial_rank in [3]:
      conv_func = tf.nn.conv3d
      if downscale=='conv_stride':
        conv_s_unit = lambda x,W: acti_func(norm_func(conv_func(x, W, [1]+[2]*spatial_rank+[1], "SAME")))
    ...
    logits = tf.compat.v1.layers.dense(tf.reshape(image,[batch_size,-1]),1,activation=None,use_bias=True)
    return logits
