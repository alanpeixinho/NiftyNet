...
class CRFTest(NiftyNetTestCase):
    def test_2d3d_shape(self):
        ...
        I = tf.random.normal(shape=[2, 4, 5, 6, 3])
        U = tf.random.normal(shape=[2, 4, 5, 6, 2])
    ...
    def test_training_3d(self):
        ...
        n_classes = 3
        # 4-features
        features = tf.random.normal(shape=[2, 8, 8, 8, n_features])
        # 3-class classification
        logits = tf.random.normal(shape=[2, 8, 8, 8, n_classes])
        # ground truth
        gt = tf.random.uniform(
            shape=[2, 8, 8, 8, n_classes], minval=0, maxval=1)

        crf_layer = CRFAsRNNLayer()
        smoothed_logits = crf_layer(features, logits)
        loss = tf.reduce_mean(input_tensor=tf.abs(smoothed_logits - gt))
        opt = tf.compat.v1.train.GradientDescentOptimizer(0.5).minimize(loss)

        with self.cached_session() as sess:
            sess.run(tf.compat.v1.global_variables_initializer())
            params = sess.run(tf.compat.v1.trainable_variables())
            for param in params:
                if param.shape == (n_classes, n_classes):
                    self.assertAllClose(param, -1.0 * np.eye(n_classes))
            sess.run(opt)
            params_1 = sess.run(tf.compat.v1.trainable_variables())
    ...
    def test_training_2d(self):
        ...
        n_classes = 3
        # 2-features
        features = tf.random.normal(shape=[batch_size, 8, 8, n_features])
        # 3-class classification
        logits = tf.random.normal(shape=[batch_size, 8, 8, n_classes])
        # ground truth
        gt = tf.random.uniform(
    ...
    def test_training_4d(self):
        ...
        with tf.device('/cpu:0'):
            ...
            smoothed_logits = crf_layer(features, logits)
        loss = tf.reduce_mean(input_tensor=tf.abs(smoothed_logits - gt))
        opt = tf.compat.v1.train.GradientDescentOptimizer(0.5).minimize(
            loss)
